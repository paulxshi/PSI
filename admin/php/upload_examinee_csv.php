...
if ($uploadError !== UPLOAD_ERR_OK) {
    ...
    $msg = $uploadErrorMessages[$uploadError] ?? "Unknown upload error (code $uploadError).";
    ob_end_clean();
    http_response_code(400);
...
// Reject files that are too large before reading into memory (10 MB ≈ 100 000 rows)
$MAX_FILE_SIZE = 10 * 1024 * 1024;
if ($file['size'] > $MAX_FILE_SIZE) {
    ob_end_clean();
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'File too large. Maximum allowed size is 10 MB (approx. 100,000 rows). Please split the file and upload in batches.']);
    exit;
}
...
// Verify that all required column headers are present in the CSV before processing any rows.
// Keys are already normalised (lowercase + underscores) by the parser above.
$firstRecord   = $csvData[0];
$hasTestPermit = array_key_exists('test_permit', $firstRecord) || array_key_exists('testpermit', $firstRecord) || array_key_exists('permit', $firstRecord);
$hasLastName   = array_key_exists('last_name',   $firstRecord) || array_key_exists('lastname',   $firstRecord);
$hasFirstName  = array_key_exists('first_name',  $firstRecord) || array_key_exists('firstname',  $firstRecord);

if (!$hasTestPermit || !$hasLastName || !$hasFirstName) {
    $missing = [];
    if (!$hasTestPermit) $missing[] = '"test_permit" (also accepted: "Test Permit", "permit")';
    if (!$hasLastName)   $missing[] = '"last_name" (also accepted: "Last Name")';
    if (!$hasFirstName)  $missing[] = '"first_name" (also accepted: "First Name")';
    ob_end_clean();
    echo json_encode([
        'success' => false,
        'message' => 'Required column(s) missing from CSV header. '
...
try {
    ...
    foreach (array_chunk($validRows, $BATCH_SIZE) as $batch) {
        // Count how many permits in this batch already exist and will be overwritten
        $permitList     = array_column($batch, 'test_permit');
        $inPlaceholders = implode(',', array_fill(0, count($permitList), '?'));
        $existsStmt     = $pdo->prepare("SELECT COUNT(*) FROM examinee_masterlist WHERE test_permit IN ($inPlaceholders)");
        $existsStmt->execute($permitList);
        $duplicateCount += (int) $existsStmt->fetchColumn();
        ...
        try {
            ...
            foreach ($batch as $row) {
                try {
                    ...
                    $successCount++;
                    // rowCount: 1 = new insert, 2 = updated existing, 0 = existed but unchanged
    ...
    // -------------------------------------------------------------------------
    // Phase 3: Send ONE batch request to the n8n welcome-email webhook.
    // Previously, up to 5 000 individual HTTP requests were made here (one per
    // row), which was the primary cause of slow response times on large uploads.
    // -------------------------------------------------------------------------
    $emailsSent   = 0;
    $emailsFailed = 0;
    ...
    if (!empty($emailQueue)) {
        ...
        if ($ch !== false) {
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($emailQueue));
            ...
            if ($httpCode === 200) {
                ...
                error_log('CSV Upload: batch welcome-email webhook OK (' . $emailsSent . ' recipients)');
            } else {
                $emailsFailed = count($emailQueue);
                error_log("CSV Upload: batch welcome-email webhook failed (HTTP $httpCode)");
            }
        } else {
            $emailsFailed = count($emailQueue);
            error_log('CSV Upload: curl_init failed for batch welcome-email webhook');
        }
    }

    $newCount = $successCount - $duplicateCount;
    $msg = "Imported $successCount record(s) successfully ($newCount new, $duplicateCount overwritten)";
